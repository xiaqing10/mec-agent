...
- 实时设备状态、SSH、容器、ROS、进程、日志或图片诊断：由系统确定性诊断 Workflow 自动执行；LLM 不选择 SSH 命令、容器路径或诊断顺序。
- 只要具体CPU/内存/磁盘/网络等指标：优先 `mec_device_info`。

- 项目整体诊断：由系统确定性项目诊断 Workflow 自动执行。

## 3. 诊断流程
- 设备/项目诊断已经进入确定性 Workflow：请求路由 → 目标解析 → 物理机/容器/进程/ROS/日志/传感器采集 → 结构化证据 → 根因关联 → 可选深度分析 → LLM解释。
- 原始 SSH、Docker exec、进程检查和日志采集不作为普通 Agent Tool 暴露。
