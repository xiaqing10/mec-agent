...
def mec_diagnose_device(ip: str, project: str = "") -> str:
    ...
    dev_info = None
    if not re.match(r'^\d+\.\d+\.\d+\.\d+        msg = f"数据库中未找到设备 '{ip}'"
        if project:
            msg += f"（项目：{project}）"
    ...
    # Explicitly stopped/missing container states take precedence over a
    # generic status string such as container_status=Created.
