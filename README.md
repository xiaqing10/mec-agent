...
MEC边缘计算设备的AI诊断与监控系统，基于 LangGraph Agent 框架，集成 RAG 知识检索增强。
...
| 版本 | 日期 | 主要更新 |
|------|------|----------|
| v4.0 | 2026-06-12 | RAG 知识检索增强、Embedding 本地化、反馈系统集成 |
| v3.4 | 2026-06-10 | MongoDB 流量查询、事件查询优化 |
| v3.0 | 2026-05 | LangGraph Agent 框架、工具系统 |
...
```
对话 → 得出结论 → 存储文本 + 向量化 → 下次类似问题检索 → 作为参考
```

- **存储的是结论文本**：如"mk18_800 根因是 zero_images"
- **向量化是为了检索**：通过语义相似度找到相关记录
- **作为参考注入 prompt**：不是直接复制，而是让 LLM 参考历史经验

### 工作流程

```
用户: "诊断一下德会项目的 mk18_800"
    ↓
1. 提取用户消息作为查询: "诊断德会项目 mk18_800"
    ↓
2. 向量化查询文本: [0.12, -0.34, ...]
    ↓
3. 在向量库中搜索最相似的记录:
   - 诊断历史: "德会 mk18_800 上次根因是 zero_images" (相似度 0.85)
   - 运维知识: "图片为0可能是相机连接异常" (相似度 0.72)
   - 修复记录: "之前重启容器后恢复" (相似度 0.68)
    ↓
4. 将检索结果注入 system prompt:
   "## 相关历史知识
    - mk18_800 上次诊断根因: zero_images
    - 图片为0可能是相机连接异常
    - 之前重启容器后恢复"
    ↓
5. LLM 基于增强后的上下文生成诊断结果
```
...
| 场景 | 没有 RAG | 有 RAG |
|------|----------|--------|
| 诊断 mk18_800 图片为0 | LLM 只能根据当前数据判断 | LLM 会参考"上次 mk18_800 也是图片为0，重启容器后恢复" |
| 排查网络故障 | LLM 只能根据通用知识回答 | LLM 会参考"之前德会项目网络故障是因为防火墙规则" |
| 修复进程崩溃 | LLM 只能给出通用建议 | LLM 会参考"之前类似问题重启 supervisor 解决了" |

### 质量过滤规则

| 数据类型 | 入库条件 | 说明 |
|----------|----------|------|
| 诊断结果 | 今天图片数 > 0 且无错误且包含"恢复正常" | 跳过（设备正常） |
| 诊断结果 | 无诊断类型、无问题描述、无错误信息 | 跳过（无明确问题） |
...
| `RAG_EMBEDDING_MODEL` | `~/.cache/huggingface/.../text2vec-base-chinese` | Embedding 模型路径 |
| `RAG_TOP_K` | `5` | 默认返回结果数 |
...
**查看知识库**：LLM 可调用 `rag_list_knowledge` 工具：
```
查看知识库中有哪些内容
```
...
```python
{
    "id": "德会_mk18_800_2026-06-02",  # 唯一标识
    "document": "项目: 德会, 设备: mk18_800...",  # 原始文本
