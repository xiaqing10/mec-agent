...
确定性诊断现在会输出统一的 `[TIMING]` 日志，用于定位“前端网络错误但后端仍在执行”这类问题。重点包括：

- `diagnose_*开始`：单次诊断入口开始时间。
- `SSH开始/完成`：单条 SSH 命令的目标、超时时间、耗时、退出码和输出大小。
- `SSH批量开始/完成`：批量采集对应的 marker 列表和整体耗时。
- `DockerExec开始/完成`：容器内命令的目标、超时时间、耗时、退出码和输出大小。
- `SensorStatus开始/完成`：传感器状态查询耗时。

排查慢请求时，优先按时间顺序查看最后一个 `[TIMING] ...开始` 与对应的 `完成` 日志；如果某一步耗时接近其 `timeout`，通常就是该步骤的主要等待点。诊断中的 ROS `rostopic hz` 等等待型命令也会通过 DockerExec/SSH 日志体现实际耗时。
...
- 对部分模型把表格数据行整体右移一列的情况做受限纠偏：仅当首列为空、后续单元格呈“设备名 + IPv4 + 数据”结构时左移一列，避免泛化修改正常空单元格。
- 对“1.**重点**”“点1.**重点**”这类编号与正文粘连的输出做受限修复：仅针对紧跟 `**` 的编号补齐列表所需空格并恢复项目换行，避免误改普通数字文本。
...
另外，回归测试现在会遍历项目内 Python 源码并用 AST 检查 `json.dumps/loads/dump/load` 等标准 JSON API：凡使用 `json` 模块却没有显式导入的文件都会直接失败，避免再次出现“某个遗漏模块运行到特定分支才触发 NameError”。`mec_diagnose_device` 与 `query_mec_device_from_db` 显式导入标准库 `json`，并通过源码回归测试同时约束 `@tool` 函数的 docstring 必须紧跟函数定义，避免因导入/注释插入 docstring 之前导致 LangChain 在启动阶段报 `Function must have a docstring`。部署代码变更后需要完整重启 Agent 进程，确保运行中的 Python 模块与当前分支代码一致。