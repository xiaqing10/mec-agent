"""Shared state and helpers for deterministic MEC diagnosis.

This module is the canonical home for per-request diagnostic progress state.
It intentionally lives under diagnose_mec so the trusted diagnosis workflow
does not depend on the LangChain tools package.
"""
...
def _build_diag_result(ip, dims, root="", project="", access=None):
    ...
    for d in dims:
        ...
        if d.get("status") in {"error", "warning"} and d.get("name"):
